...
rule integrityOfSwap(env e, bytes data) {
    ...
    assert data.length == 0 => after0  == before0  + amount0Out - amount0In;
    assert data.length == 0 => after1  == before1  + amount1Out - amount1In;
    assert data.length == 0 => r0After == r0Before + amount0In  - amount0Out;
    assert data.length == 0 => r1After == r1Before + amount1In  - amount1Out;
...
rule integrityOfProtocolFee(env e,method f, calldataarg args) filtered {
    ...
    uint Sm = assert_uint256((totalSupplyBefore * (rootK - rootKLast)) / ((rootK * 5) + rootKLast));

    assert feeToOn               <=> protocolFeeMargin == Sm;
