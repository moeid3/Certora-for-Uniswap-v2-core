...
{   
    ...
    calldataarg args1; f(e1, args1);

    uint before = _pool.kLast;

    env e; calldataarg args; swap(e, args);

    // setup(e2);
    // routerHandlerFnCall(e2, g);
    calldataarg args2; g(e2, args2);

    uint after = _pool.kLast;

    // y0/x0 == y1/x1
    assert _factory.feeTo() != 0 => before == after, "Prices are not equal";
}
...
rule onlyAssociatedFn(env e, method f) filtered { f -> onlyPoolMethods(f) } {
    ...
    f.selector == sig:burn(address).selector => routerHandlerFnCall(e, f);
