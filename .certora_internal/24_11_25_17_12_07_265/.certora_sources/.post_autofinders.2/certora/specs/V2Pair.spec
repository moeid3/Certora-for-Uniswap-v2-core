...
rule integrityOfSwap(env e, bytes data) {
    ...
    assert data.length == 0 => (after0  == before0  + amount0Out - amount0In)  && (after1  == before1  + amount1Out - amount1In);
    assert data.length == 0 => (r0After == r0Before + amount0In  - amount0Out) && (r1After == r1Before + amount1In  - amount1Out);
...
rule neverGetZero(env e, method f, calldataarg args) filtered { f -> onlyPoolMethods(f) } {
    ...
    require totalSupply() >= MINIMUM_LIQUIDITY && balanceOf(0) == MINIMUM_LIQUIDITY;
    require balanceOf(0) + balanceOf(e.msg.sender) <= totalSupply();
