...
function setup(env e) {
    ...
    // Note that the prover can't find a path where both of them are true
...
function cvlTransfer(env passedENV, address token, address to, uint256 value) {
    ...
    require to != _pool;
    assert value <= token.balanceOf(passedENV, _pool);
    token.transferFrom(passedENV, _pool, to, value);
}


function cvlAddLiquidity(uint amount0Desired, uint amount1Desired) returns (uint, uint) {
    uint reserve0; uint reserve1;
    (reserve0, reserve1, _) = getReserves();
    if (reserve0 == 0 && reserve1 == 0) {
            return (amount0Desired, amount1Desired);
    } else {
        uint amount1Optimal = cvlQuote(amount0Desired, reserve0, reserve1);
        if(amount1Optimal <= amount1Desired) {
            return (amount0Desired, amount1Optimal);
        } else {
            uint amount0Optimal = cvlQuote(amount1Desired, reserve1, reserve0);
...
function cvlQuote(uint amount0, uint reserve0, uint reserve1) returns uint {
    require reserve0 > 0;
...
function routerHandlerFnCall(env e, method f) {

    if(f.selector == sig:mint(address).selector) {
        uint amount0Desired; uint amount1Desired;
        uint amount0; uint amount1;
        // for avoiding rounding errors
        require amount0Desired >= WAD() && amount1Desired >= WAD();
        (amount0, amount1) = cvlAddLiquidity(amount0Desired, amount1Desired);
    ...
    } else if(f.selector == sig:burn(address).selector) {
        uint pool0Before           = _token0.balanceOf(_pool);
        uint pool1Before           = _token1.balanceOf(_pool);
        uint callerLiquidityBefore = balanceOf(e.msg.sender);
...
// function _pool.getAmountIn(uint amountOut, uint112 reserve0, uint112 reserve1) returns uint {
//     uint numerator = require_uint256(reserve0 * amountOut * 1000);
//     uint denominator = require_uint256((reserve1 - amountOut) * 997);
//     require denominator != 0;
//     return assert_uint256((numerator / denominator) + 1);
// }
...
function cvlUniswapV2Call(env passedENV, address sender, uint amount0, uint amount1, bytes data) {
    ...
    if(amount0 > 0  /* token0 */) {
        // amountIn
        // TODO: time out
        uint amountToRepay = _pool.getAmountIn(amount0, r1, r0);
    ...
    if(amount1 > 0  /* token1 */) {
        // TODO: time out
        uint amountToRepay = _pool.getAmountIn(amount1, r0, r1);
...
*/
rule integrityOfSwapWithoutFlashLoan(env e, bytes data) {

    setup(e);
    require data.length == 0 || data.length == 1;
    ...
    uint amount0In = _pool.getAmountIn(amount0Out, r1Before, r0Before);
    uint amount1In = _pool.getAmountIn(amount1Out, r0Before, r1Before);
    ...
    assert (after0  == before0  + amount0Out - amount0In)  && (after1  == before1  + amount1Out - amount1In);
    assert (r0After == r0Before + amount0In  - amount0Out) && (r1After == r1Before + amount1In  - amount1Out);

    // y1*x1 > y0*x0
    assert r1After * r0After > r1Before * r0Before;
}

rule integrityOfSwapWithFlashLoan(env e, bytes data) {
...
/* DONE
Price of AMM must be the same before and after any operations (except swap).
        (reserve1 / reserve0)ᵖʳᵉ == (reserve1 / reserve0)ᵖᵒˢᵗ
        (reserves0 ⋅ reserves1)ᵖʳᵉ =(reserves0 ⋅ reserves1)ᵖᵒˢᵗ
*/
rule integrityOfMainInvariant(env e, method f) filtered 
{                      // reserves increase on swap
    ...
    // y0/x0 == y1/x1
    assert after0 != 0 && before0 != 0 => rangeDivEqual(before1, before0, after1, after0), "Prices are not equal";
}
...
rule integrityOfProtocolFee(env e,method f, calldataarg args) filtered {
    ...
    uint protocolFeeMargin = require_uint256(balanceOf(_factory.feeTo()) - balanceBefore);

    assert feeToOn => protocolFeeMargin == require_uint256((totalSupplyBefore * (rootK - rootKLast)) / ((5 * rootK) + rootKLast));
}
...
/*
After the first mint, total supply and reserves never get zero.
*/
rule neverGetZero(env e, method f, calldataarg args) filtered { f -> onlyPoolMethods(f) } {
    uint256 MINIMUM_LIQUIDITY = 1000;
    require balanceEqualReserve() && e.msg.sender != _pool && e.msg.sender != 0;

    f(e, args);

    assert totalSupply()  >= MINIMUM_LIQUIDITY;
    assert _pool.reserve0 != 0 && _pool.reserve1 != 0;
