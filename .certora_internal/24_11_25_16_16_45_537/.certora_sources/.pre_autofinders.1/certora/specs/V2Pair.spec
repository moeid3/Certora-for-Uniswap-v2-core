...
// function cvlGetAmountIn(uint amountOut, uint112 reserve0, uint112 reserve1) returns uint {
...
function cvlUniswapV2Call(env passedENV, address sender, uint amount0, uint amount1, bytes data) {
    ...
    if(amount0 > 0  /* token0 */) {
        // amountIn
        // TODO: time out
        uint amountToRepay = cvlGetAmountIn(amount0, r1, r0);
    ...
    if(amount1 > 0  /* token1 */) {
        // TODO: time out
        uint amountToRepay = cvlGetAmountIn(amount1, r0, r1);
...
rule integrityOfSwapWithoutFlashLoan(env e, bytes data) {
    ...
    uint amount0In = cvlGetAmountIn(amount0Out, r1Before, r0Before);
    uint amount1In = cvlGetAmountIn(amount1Out, r0Before, r1Before);
