...
methods {
    unresolved external in _._ => DISPATCH [
        ...
        _.uniswapV2Call(address,uint,uint,bytes),
        DummyERC20A._,
        DummyERC20B._,
        UniswapV2Pair._,
...
// TODO: use this for main invariant rule issue!!!
...
function routerHandlerFnCall(env e, method f) {
    ...
    } else if(f.selector == sig:swap(uint,uint,address,bytes).selector) {
        uint112 r0Before; uint112 r1Before;
        (r0Before, r1Before, _) = getReserves();
        
        uint amount0Out; uint amount1Out; bytes data;
        // A flag indicating a swap operation with or without flash loan
...
//     // Passed env caller is the pool contract
...
//     // We need to ensure that the function repays the flash loan with fee to the pool
...
*/
rule integrityOfSwapWithoutFL(env e, bytes data) {
...
rule integrityOfSwapWithFL(env e, bytes data) {
...
// */
// rule integrityOfMainInvariant(env e, calldataarg args, method f) {
...
//     // y0 * x0 == y1 * x1
//     assert f.selector != sig:swap(uint,uint,address,bytes).selector => r1Before / r0Before == r1After / r0After, "Prices are not equal";
// }
...
rule integrityOfTWAP(env e, method f, calldataarg args) filtered {
    ...
    assert (
        safeSub(after0, before0) == encodePrice(r0, r1) * timeElapsed &&
        safeSub(after1, before1) == encodePrice(r1, r0) * timeElapsed ),
        "Wrong Price cumulative";
}
...
rule integrityOfProtocolFee(env e,method f, calldataarg args) filtered {
    ...
    assert feeToOn => protocolFeeMargin == assert_uint256(
        (totalSupplyBefore * (rootK - rootKLast)) / ((rootK * 5) + rootKLast)
        );
}
