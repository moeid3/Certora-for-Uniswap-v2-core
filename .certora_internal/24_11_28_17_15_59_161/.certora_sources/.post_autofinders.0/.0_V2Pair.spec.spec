...
methods {
    ...
    function _.uniswapV2Call(address sender, uint amount0, uint amount1, bytes) external with (env e) => cvlUniswapV2Call(e, sender, amount0, amount1) expect void;
...
function cvlUniswapV2Call(env passedENV, address sender, uint amount0, uint amount1) {
    // Passed env is the pool contract

    // Proving the integrity of entrant and lock() modifier
    assert _pool.unlocked == 0;

    // This is just a flag to indicate a swap operation 
    // with or without the flash loan to cover all the scenarios
    bytes individualData;
    require individualData.length == 0 || individualData.length == 1;

    // Makes arbitrary calls back to the v2 pair to cover all the scenarios.
    _flr.uniswapV2Call(passedENV, individualData);
...
rule integrityOfSwapWithoutFL(env e, bytes data) {
    ...
    assert amount0In == r0After - r0Before + amount0Out;
    assert amount1In == r1After - r1Before + amount1Out;
...
*/
rule integrityOfMainInvariant(method f, method g) filtered 
{   
    f -> f.selector == sig:burn(address).selector ||
         f.selector == sig:mint(address).selector
    // g -> g.selector == sig:burn(address).selector ||
        //  g.selector == sig:mint(address).selector
} {
    // TODO: reAssess
    ...
    mathint before = r0Before * r1Before;
    ...
    mathint after = r0After * r1After;

    // y0 * x0 == y1 * x1
    assert before != 0 && after != 0 => before == after, "Prices are not equal";
}
